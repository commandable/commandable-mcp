...
suiteOrSkip('google-workspace docs write handlers (live)', () => {
  for (const variant of variants) {
    describe(`variant: ${variant.key}`, () => {
      ...
      it('docs_batch_update can perform a trivial replaceAllText no-op', async () => {
        const documentId = ctx.documentId
        if (!documentId)
          return expect(true).toBe(true)
        const docs_batch_update = docs.write('docs_batch_update')
        const res = await docs_batch_update({ documentId, requests: [
