    ...
    variants: {
      variants: {
        api_token: {
          ...
          injection: { headers: { Authorization: 'Basic {{base64(email + ":" + apiToken)}}', Accept: 'application/json' } },
    ...
    variants: {
      variants: {
        api_token: {
          ...
          baseUrlTemplate: 'https://{{domain}}.atlassian.net',
          injection: { headers: { Authorization: 'Basic {{base64(email + ":" + apiToken)}}', Accept: 'application/json' } },
          healthCheck: { path: '/wiki/rest/api/user/current' },
        },
        oauth_token: {
    ...
    variants: {
      variants: {
        service_account: {
          ...
          injection: { headers: { Authorization: 'Bearer {{token}}' } },
          preprocess: 'google_service_account',
          healthCheck: { path: '/files?pageSize=1&fields=files%28id%29' },
        },
        oauth_token: {
          label: 'OAuth Access Token (short-lived)',
          schema: {
