...
suite('google-workspace sheets write handlers (live)', () => {
  const liveCoverage = createLiveToolCoverage(getPlanEntry('google-workspace-sheets-write'))
  ...
  it('sheets_batch_update handles a trivial request (no-op findReplace)', async () => {
    if (!ctx.spreadsheetId)
      return expect(true).toBe(true)
    const aTitle = sheetTitle || 'Sheet1'
    const sheets_batch_update = sheets.write('sheets_batch_update')
    const res = await sheets_batch_update({ spreadsheetId: ctx.spreadsheetId, requests: [
